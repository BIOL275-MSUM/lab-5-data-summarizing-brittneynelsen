...
> B. Rename each variable so that it is all lower-case and uses an
...
> C. Convert the four numerical variables from mm to cm by multiplying
> by 10. Print the resulting table.
...
> D. Calculate sepal area and petal area (area is equal to length
...
> E. Calculate sepal area and petal area (area is equal to length
> multiplied by width). Print a table with only the variables sepal
> area, petal area, and species. - sample size - maximum value - minimum
> value - range - median - first quartile (q1) - third quartile (q2) -
...
> F. Calculate the following statistics for each species from the petal
...
> G. Visualize the relationship between petal length and species using a
> strip plot.
...
> H. Starting with the previous graph, add the mean and 95% confidence
...
> I. Visualize the relationship between petal length, petal width, and
